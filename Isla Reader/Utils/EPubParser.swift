...
class EPubParser {
    ...
    private static func mapTOCEntriesToChapters(_ tocEntries: [TOCEntry], hrefToChapterIndex: [String: Int]) -> [TOCItem] {
        ...
        let filteredItems = filterMeaninglessNestedTOCItems(items)
        if filteredItems.count != items.count {
            DebugLogger.info("EPubParser: 目录过滤无意义子条目 \(items.count - filteredItems.count) 个")
        }
        return filteredItems
    }

    private static func filterMeaninglessNestedTOCItems(_ items: [TOCItem]) -> [TOCItem] {
        guard items.count > 1 else { return items }

        var filtered: [TOCItem] = []
        filtered.reserveCapacity(items.count)

        for (index, item) in items.enumerated() {
            if shouldDropMeaninglessNestedTOCItem(item, at: index, in: items) {
                continue
            }
    ...
    private static func shouldDropMeaninglessNestedTOCItem(_ item: TOCItem, at index: Int, in items: [TOCItem]) -> Bool {
        let normalizedTitle = item.title.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedTitle.isEmpty else { return false }
        guard item.level > 0 else { return false }
        guard isLikelyMeaninglessTOCTitle(normalizedTitle) else { return false }
        guard isLikelyAutoGeneratedFragment(item.fragment) else { return false }

        let itemPath = normalizeTOCHrefPath(item.href)
        guard let parent = nearestParentItem(for: index, in: items) else { return false }
        guard !isLikelyMeaninglessTOCTitle(parent.title.trimmingCharacters(in: .whitespacesAndNewlines)) else {
            return false
        }

        let parentPath = normalizeTOCHrefPath(parent.href)
        let pointsToSameChapter = parent.chapterIndex == item.chapterIndex || parentPath == itemPath
        return pointsToSameChapter
    }

    private static func nearestParentItem(for index: Int, in items: [TOCItem]) -> TOCItem? {
    ...
    private static func isLikelyAutoGeneratedFragment(_ fragment: String?) -> Bool {
    ...
    private static func isLikelyMeaninglessTOCTitle(_ title: String) -> Bool {
        let normalized = title.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalized.isEmpty else { return true }

        if normalized.unicodeScalars.allSatisfy({ CharacterSet.decimalDigits.contains($0) }) {
            return true
        }

        let romanNumerals = CharacterSet(charactersIn: "ivxlcdmIVXLCDM")
        if normalized.unicodeScalars.allSatisfy({ romanNumerals.contains($0) }) {
            return true
        }

        let chineseNumerals = CharacterSet(charactersIn: "零〇一二三四五六七八九十百千万两壹贰叁肆伍陆柒捌玖拾佰仟")
        if normalized.unicodeScalars.allSatisfy({ chineseNumerals.contains($0) }) {
            return true
        }
