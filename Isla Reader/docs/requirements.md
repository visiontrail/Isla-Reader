...
### 关键功能清单
- 本地导入：从“文件”App/本机导入 ePub/纯文本并解析。
...
- 账号与同步：无需注册，数据仅存储于本地设备；不提供跨设备云同步。
...
- 账号与同步
  - 功能：无需注册；数据仅保存在本地；提供导入/导出与本地数据删除入口。
  - 验收：数据操作仅影响当前设备；本地导出/删除可正常工作；不存在跨设备同步行为。
...
- 数据存储：本地 Core Data；无需注册；不提供云同步。
- 实时交互：WebSocket/Server-Sent Events 用于流式与实时互动。

### 数据模型草案（简化）
- User（本地设置）：id，settings（不采集邮箱）；数据仅存储在本地。
...
- 用户数据仅存储在本地；无应用内注册；提供本地数据删除与导出入口。
