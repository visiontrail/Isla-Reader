...
- Xcode 工程当前 Marketing Version：`1.0.5`
- 主要导入格式：通过“文件”App 导入 `EPUB`
- App 内置 UI 语言：英文、简体中文、日文、韩文
- 脚本默认模拟器（不传参数时）：`iPhone 15 Pro`

## 最近进展（2026-03 快照）

- 阅读器稳定性提升：优化文本选择、页面边缘交互与横向漂移控制。
- 高亮工作流升级：支持多选合并、笔记编辑行为对齐，以及超大分享卡片的导出兜底。
- AI 交互升级：`Ask AI` 输入框支持多行自适应高度，翻译/解释/提问动作顺序与交互统一。
- 指标统计细化：按本地时区对齐 7 天/周/月窗口，并拆分摘要与略读场景的阅读/AI 指标。
- 略读流程优化：激励插屏展示时机延后到章节切换稳定后，并补充更清晰的展示提示。

## 验证快照（2026-03-31）

- `Isla ReaderTests`：17 个测试文件，59 个 `@Test` 用例。
- `Isla ReaderUITests`：2 个 UI 冒烟测试文件。
- `server/tests`：5 个后端 pytest 文件。
...
### 环境要求
- Xcode 15+
- iOS 模拟器运行时（推荐设备：`iPhone 16`；不传设备参数时脚本默认 `iPhone 15 Pro`）
- macOS 命令行工具
- 后端可选：Python 3.11+
